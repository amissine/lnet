...
function LocalizedNetworking( options ) {
  ...
  const hs      = options.hs  || "~/project/lnet/bin/hub_hb1.js"
  ...
  function ssh_connect( hub ) {
    return new Promise(resolve => {
      ...
      ssh.on('close', code => {   // Dim                                               Reset
        code === 255 && console.log("\x1b[2mssh_connect: pid " + ssh.pid + ", timed out\x1b[0m") == undefined
        || console.log("\nssh_connect: pid " + ssh.pid + ", exit code " + code) // TODO remove this line
      })
      ssh.stdout.on('data', data => {                                  // Dim        Reset
        console.log(`%sssh_connect: pid ${ssh.pid}, received ${data}%s`, "\x1b[2m", "\x1b[0m")
        resolve(false)
      })
