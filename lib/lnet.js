...
function LocalizedNetworking( options ) {
  ...
  async function loop2connect2( hub ) {
    ...
    while (hub.loop) {
      if (hub.disconnected) { // Start SSH; on exit, kill the SSH process; on close, remove the killOnExit listener
        hub.ssh = spawn("ssh", [ "-fR", `${llp}:127.0.0.1:${lp}`, hub.ip, hs, hub.ip ])
  ...
  function ssh_connect( hub ) {
    var hbtoHandle = null
    console.log("\x1b[2mssh_connect: pid " + hub.ssh.pid + ", connecting to " + hub.ip + "\x1b[0m")
             //  Dim                                                                      Reset
