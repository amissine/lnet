...
function LocalizedNetworking( options ) {
  ...
  const toReconnect = options.toReconnect || +10000
  ...
  // Start looping to connect to the hub
  self.hub = context.hub
  console.log("Starting remote port forwarding loop for hub.ip=" + self.hub.ip + " hub.llp=" + self.hub.llp + " hub.lp=" + self.hub.lp)
  loop2connect2(self.hub).then(data => { console.log(data); return })
  ...
  function ssh_connect( hub ) {
    ...
    return new Promise(resolve => {
      ...
      hub.ssh.stderr.on('data', data => { // Dim    FgRed       Reset
        ...
        || resolve(true) == undefined && rl.close()
      })
      return
    })
  }
