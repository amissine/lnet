  ...
  function onSshStdout(hub, data) {
    ...
    if (typeof data == 'object') {
      if (data.heartbeat) { // respond with a leaf heartbeat
        ...
        /* bus.publish(data) && */ broadcast(hub, str)
      }
      return;
...
/* Old version - DO NOT REMOVE UNTIL git pull WORKS AGAIN!
...
/*
  function wrap( data, wObj ) {
    var str = data.toString().trim(), jsObject, ok = true
    try {
      jsObject = JSON.parse(str)
    } catch (e) {
      ok = false
    }
    if ( ok && jsObject.data && typeof jsObject.data == 'string' ) {
      jsObject.data = wrap(jsObject.data, {}).data
    }
