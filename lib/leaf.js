...
function Leaf( options ) {
  ...
  function onSshStdout(hub, jsObject) {
    ...
    if (jsObject.jso == true) {
      if (jsObject.data.heartbeat) { // respond with a leaf heartbeat
        ...
        cp.exec('sudo su - ctl -c \'echo "' + git + '" >> /tmp/ctlsvc_admin\'') // TODO: Fix hardcoded CTLSVC name
      }
      return;
    }
  ...
  function addSshListeners( hub ) {
    ...
    hub.ssh.on('close', code => { 
      log(ms() + "hub.ssh.pid=" + hub.ssh.pid + " event close, code=" + code)
      self.removeListener("exit", hub.koe)
      self.emit('disconnect')
    })
    hub.ssh.stdout.on('data', data => {
      onSshStdout(hub, wrap(data, { hub_llp: +hub.llp }))
//      hub.ondata(hub.resolve, data.indexOf('"heartbeat"') > 0 ? JSON.parse(data) : data)
    })
    hub.ssh.stderr.on('data', data => { 
      onSshStderr(hub, wrap(data, {}))
/*      data.indexOf("No such file or directory") > 0 && (hub.loop = false) == false && hub.resolve(false) == undefined
      || data.indexOf("remote port forwarding failed for listen port") > 0 && (hub.loop = false) == false && hub.resolve(false) == undefined
      || data.indexOf("No route to host") > 0 && setTimeout(() => hub.resolve(true), nr2hDelayMs) != null
      || data.indexOf("Operation timed out") > 0 && setTimeout(() => hub.resolve(true), wait2reconnect) != null
      || data.indexOf("Allocated port") == 0 && hub.ondata(hub.resolve, data.toString())
      || data.indexOf("Killed by signal 15.") == 0 && hub.resolve(true) == undefined
      || hub.resolve(true) // == undefined && rl.close() // rl.close() means exit */
    })
  }
  ...
  function onSshStderr(hub, jsObject) {
    ...
    if (jsObject.data.indexOf('Allocated port') == 0) {
  ...
  function wrap( data, wObj ) {
    ...
    wObj.time = Date.now(); wObj.jso = ok; wObj.data = ok ? jsObject : str
    return wObj;
  }
...
/*    function ondata(resolve, line) {
      ...
      if (line.heartbeat) { // received a heartbeat from hub, respond with a leaf heartbeat
        ...
        cp.exec('sudo su - ctl -c \'echo "' + line + '" >> /tmp/ctlsvc_admin\'') // TODO: Fix hardcoded CTLSVC name
      }
      hub.heartbeatMonitor.emit("inbound_data")
      return true
    }; hub.ondata = ondata */
