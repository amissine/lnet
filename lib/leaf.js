...
function Leaf( options ) {
  ...
  // Start ====================================================================
  log(`process.env.gitdesc=${process.env.gitdesc}`)
  ...
  function dispatch( data ) {
    ...
    if (q.message && q.message.indexOf('git pull') == 0) { var git = q.message
      ...
      if (process.env.gitdesc < version) { // TODO: fix version comparison
        log("Time to 'git pull' and restart")
      } else if (process.env.gitdesc == version) {
        log("We're good")
      }
    }
    return true;
