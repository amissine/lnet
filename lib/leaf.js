...
function Leaf( options ) {
  ...
  async function loop2read_stdin() {
    while (rl_loop) { rl_loop = await rl_stdin() }
  ...
  async function loop2connect2( hub ) {
    ...
    function killOnExit() {
      if (hub.ssh) {
        log("killOnExit: hub.ssh.pid=" + hub.ssh.pid); hub.ssh.kill();
      }
    }
    ...
    return "loop2connect2("+hub.ip+"): UNEXPECTED error"
  }
  ...
  function ssh_disconnected( hub ) {
    return new Promise(resolve => {
      self.once("disconnected_" + hub.ip, () => {
        console.log(ms() + "ssh_disconnected: killing hub.ssh.pid=" + hub.ssh.pid); hub.ssh.kill()
        setTimeout(() => resolve(true), wait2reconnect); return
      })
    })
  }
  ...
  function ssh_connect( hub ) {
    var hbtoHandle = null
    log(ms() + "hub.ssh.pid=" + hub.ssh.pid + " connecting to " + hub.ip)
    function ondata(resolve, line) {
      ...
      if (line.indexOf("heartbeat") > 0) {
        ...
        hbtoHandle = setTimeout(() => self.emit("disconnected_" + hub.ip), context.heartbeatRateMs * 1.5)
    ...
    return new Promise(resolve => { // resolve(true) means hub.disconnected
      hub.ssh.stdout.on('data', data => {
        console.log(ms() + "hub.ssh.pid=" + hub.ssh.pid + " event data='" + data + "'")
        ondata(resolve, data)
      })
