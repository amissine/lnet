...
function Leaf( options ) {
  ...
  self.on('disconnect', () => {
    if (self.exiting && --self.count == 0) {
      ...
      var command2restart = `pipe="/tmp/${USER}_2hubs.in"; rm $pipe 2>/dev/null; mkfifo $pipe; cat $pipe | bin/lnet.js "${config}" >> "/tmp/${USER}_2hubs.out" 2>&1 &`
  ...
  rl.on("close", () => { 
    log(ms() + "rl.on('close'): process.argv=" + util.inspect(process.argv) + "\n")
  ...
  function dispatch( data ) {
    ...
    log(util.inspect(q))
    if (q.message == 'git pull origin master') { var git = q.message
  ...
  function onSshStdout(hub, data) {
    ...
    if (typeof data == 'object') {
      if (data.heartbeat) { // respond with a leaf heartbeat
        ...
        dispatch(data) && broadcast(hub, str)
      }
      return;
  ...
  function addSshListeners( hub ) {
    ...
    hub.ssh.stdout.on('data', data => onSshStdout(hub, parse(data)))
    hub.ssh.stderr.on('data', data => onSshStderr(hub, parse(data)))
  }

  function parse( data ) {
    if ( typeof data != 'string' ) data = data.toString().trim() // Buffer
    try {
  ...
  function broadcast(hSelf, str) {
    context.hubs.forEach((hub) => { if (hub === hSelf) return;
