...
function Leaf( options ) {
  ...
  // Start ====================================================================
  log(`process.env.gitdesc=${process.env.gitdesc} process.env.IO_SUFFIX=${process.env.IO_SUFFIX}`)
  ...
  self.on('disconnect', () => {
    if (self.exiting && --self.count == 0) {
      ...
      var command2restart = `pipe="/tmp/${USER}${process.env.IO_SUFFIX}.in"; rm $pipe 2>/dev/null; mkfifo $pipe; cat $pipe | gitdesc=\`git describe\` IO_SUFFIX=${process.env.IO_SUFFIX} bin/lnet.js "${config}" >> "/tmp/${USER}${process.env.IO_SUFFIX}.out" 2>&1 &`
  ...
  function dispatch( data ) {
    ...
    if (q.message && q.message.indexOf('git pull') == 0) {
      ...
      if (semver.lt(process.env.gitdesc, version) || gitCommitLt(process.env.gitdesc, version)) {
        ...
        gitPull('git pull --tags origin master; IO_SUFFIX='+process.env.IO_SUFFIX)
      }
    }
    return true;
  }
