...
function Hub( options ) {
  ...
  var rl_cb = function( line ) {
    ...
    if (line.indexOf('"heartbeat"') > 0) {                 // heartbeat
      if (!llp) {
        var heartbeat = JSON.parse(line)
        llp = heartbeat.llp; hubIPC = new HubIPC({llp: llp, gitdesc: heartbeat.heartbeat, hub: self})
      }
      return true
    }
    ...
    hubIPC && hubIPC.emit('broadcast', line); return true; // some other data
  }
