...
function Hub( options ) {
  ...
  const ctlsvc_name      = options.ctlsvc_name ? options.ctlsvc_name : 'ctlsvc_alec' 
  const self             = this
  ...
  // Read the leaf data (a heartbeat, a command, or some json to broadcast) from stdin (which is the stdout of the remote leaf)
  var rl_cb = function( line ) {
    ...
    hubIPC.emit('broadcast', line); return true;
  }
  ...
  function command( line ) {
    out("Hub llp=" + llp + " executing '" + line + "'")
    if (line == 'disconnect') {
      rl.emit('close'); return true
    }
    if (line == "stop heartbeat") {
      ...
      if (line.indexOf('git pull origin master') >= 0) { // For now, no pub/sub on hubs
        child_process.exec('echo "git pull origin master" >> /tmp/' + ctlsvc_name)
        out('Execution started')
      }
      hubIPC.emit('broadcast', line)
      return true
/*    } else if (line == 'git pull origin master') {
      child_process.exec('echo "' + line + '" >> /tmp/' + ctlsvc_name) 
      hubIPC.emit('broadcast', line); out('Execution started');
