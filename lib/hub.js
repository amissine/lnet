...
function Hub( options ) {
  ...
  const self             = this
  const heartbeatMonitor = new HeartbeatMonitor( { mock: options.mock,
    remoteEnd: self, eventName: "close", timeoutMs: heartbeatRateMs * 1.5 } )
  ...
  self.on('close', () => { cleanupAndExit(0) }) // new connection from a remote leaf has been accepted
  ...
  var hbLoop = setInterval(() => { console.log("\x1b[2m" + hbMsg() + "\x1b[0m") }, heartbeatRateMs)
  console.log("\x1b[2m"+hbMsg()+"\x1b[0m")   // Dim                   Reset
