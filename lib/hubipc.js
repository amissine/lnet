...
function HubIPC( options ) {
  ...
  const version   = options.gitdesc // `git describe` of the leaf
...
//    debug('checkRegistryOnEnd LOCK write='+result)
  ...
  function runServer(path, wrapper) {
    fs.stat(path, (err, stats) => {
      if (err != null && err.code != 'ENOENT') { // file exists, remove it
        console.error("UNEXPECTED 67 path=" + path + ", err="  + util.inspect(err))
        fs.unlink(path, e => {
          console.error("UNEXPECTED 69 path=" + path + ", e="  + util.inspect(e))
          close()
        })
      }
    })
...
//            console.error('Lock acquired id='+id)
  ...
  acquireRegistryLock().then(() => addHubServer()).then(() => broadcast(`Hello ${version}`))
  .then(() => releaseRegistryLock())
}
/*
 * ACKNOWLEDGMENTS
