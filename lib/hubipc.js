...
//    debug('remHubServer started')
  ...
  async function addHubServer() {
    ...
    is.ondata = (socket, data) => {
      const port = data.slice(0, data.indexOf(' ')), str = data.slice(data.indexOf(' ') + 1)
      if (port == id) return; // ignore data from self
      console.log(JSON.stringify({ llp: +port, to: +id, data: wrap(str, {}) }))
    }
...
//    debug('broadcast started')
    ...
    fs.readdir(REGISTRY, (err, files) => {
      ...
      files.forEach(v => { if (v == REGFILE || v.slice(0, '.') == id) return;
        try {
